...
export class ApiService {
  ...
  updateTravelStatus(userName, travelId, status) {
    let body = new URLSearchParams();
    body.set('action', 'VASetTravelRequestStatus');
    body.set('user', userName);
    body.set('travelId', travelId);
    body.set('status', status);

    const res = this.http.post("http://bt.the-v.net/service/api.aspx", body, this.options).toPromise();
