...
export class AmbassadorPartnersPage implements OnInit {
  ...
  ngOnInit() {
    this.sqlSvc.getAmbassadorsData({ title: 'VP' }).then(val => {
      ...
      this.vps.forEach(item => {
        ...
        // console.log(this.imageCorrection(item['imgUrl']))
  ...
  async imageCorrection(image) {
    ...
    let call = this.nativeHttp.sendRequest(image, { method: 'get', responseType: 'blob', data: {}, headers: {} })
    // .then(result => {
    //   let data;
    //   console.log(result)
    ...
    from(call).pipe().subscribe(result => {
      let data;
      console.log(result)
      reader.readAsDataURL(result.data);
      reader.onloadend = function () {
